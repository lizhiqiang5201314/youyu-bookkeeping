            ...
            for await (const event of anthropicStream) {
                if (event.type === "message_start") {
                    ...
                    if (event.delta.type === "text_delta") {
                        ...
                        const index = blocks.findIndex((b) => b.index === event.index);
                        const block = blocks[index];
                        if (block && block.type === "toolCall" && event.delta.partial_json.length > 0) {
...
function createClient(model, apiKey, interleavedThinking, optionsHeaders, dynamicHeaders) {
    ...
    const isKimiProvider =
        (model.provider || "").toLowerCase() === "kimi" ||
            (model.id || "").toLowerCase().includes("kimi") ||
            (model.baseUrl || "").toLowerCase().includes("kimi");
    ...
    // Kimi frequently provides full tool args in content_block_start without follow-up JSON deltas.
    // Leaving fine-grained tool streaming enabled makes those calls collapse to {} later in the stream.
    const betaFeatures = isKimiProvider ? [] : ["fine-grained-tool-streaming-2025-05-14"];
    if (needsInterleavedBeta) {
        betaFeatures.push("interleaved-thinking-2025-05-14");
    }
